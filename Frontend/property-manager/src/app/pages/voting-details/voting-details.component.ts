...
export class VotingDetailsComponent implements OnInit, OnDestroy {
  ...
  public selectedSpecializations = signal<string[]>([]);
  public filteredContractors = signal<AssignedContractor[]>([]);
    ...
    if (this.taskId) {
      ...
      this.taskService.getTaskById(this.taskId).subscribe({
        next: async (res) => {
          ...
          this.filteredContractors.set(this.contractors() || []); // Initialize filtered contractors
            ...
            if(res.taskUuid)
            {
              ...
              this.taskService.getTaskById(this.taskId).subscribe({
                next: (res) => {
                  ...
                  //Get voting ends at time
                  this.votingService.getSessionFromTaskId(this.taskId!).subscribe({
                    next: (ses) => {

                      const votingEndsDate = new Date(
                        ses.votingEndsAt[0],
                        ses.votingEndsAt[1] - 1,
                        ses.votingEndsAt[2], 
                        ses.votingEndsAt[3], 
                        ses.votingEndsAt[4]
                      );
                      ...
                      if (this.task()) {
                        this.task()!.scheduled_date = votingEndsDate;
                      }
  ...
  private addToContractors(contractor: AssignedContractor) {
    ...
    this.filterContractors(); // Update filtered contractors when a new contractor is added
  ...
  filterContractors() {
    const selectedSpecs = this.selectedSpecializations();
    ...
    const filtered = (this.contractors() || []).filter(contractor =>
      contractor.specializations?.some(spec => selectedSpecs.includes(spec))
    );
