...
class BodyCorporateService(
    ...
    fun login(request: LoginRequest): LoginResponse {
        val bodyCorporate = bodyCorporateRepository.findByEmail(request.email)
    ...
    fun getAllBodyCorporates(pageable: Pageable): Page<BodyCorporateResponse> {
        return bodyCorporateRepository.findAll(pageable)
            .map { it.toResponse() }
    }
    ...
    fun getBodyCorporateByEmail(email: String): BodyCorporateResponse {
        val bodyCorporate = bodyCorporateRepository.findByEmail(email)
    ...
    fun getBodyCorporateByUserId(userId: String): BodyCorporateResponse {
        val bodyCorporate = bodyCorporateRepository.findByUserId(userId)
    ...
    fun updateBodyCorporate(id: UUID, request: UpdateBodyCorporateRequest): BodyCorporateResponse {
        val existingBodyCorporate = bodyCorporateRepository.findById(id)
            .orElseThrow { NoSuchElementException("Body corporate not found with id: $id") }
        ...
        val updatedBodyCorporate = existingBodyCorporate.copy(
    ...
    fun searchBodyCorporatesByName(name: String): List<BodyCorporateResponse> {
        return bodyCorporateRepository.findByCorporateNameContainingIgnoreCase(name)
            .map { it.toResponse() }
    }
    ...
    fun getBodyCorporatesByContributionRange(
        ...
        return bodyCorporateRepository.findByContributionPerSqmBetween(minContribution, maxContribution)
            .map { it.toResponse() }
    }

    @Transactional(readOnly = true)
    fun getBodyCorporatesByMinimumBudget(minBudget: BigDecimal): List<BodyCorporateResponse> {
        return bodyCorporateRepository.findByTotalBudgetGreaterThanEqual(minBudget)
            .map { it.toResponse() }
    }
