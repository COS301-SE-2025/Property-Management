...
class BodyCoporateControllerTest(@Autowired val mockMvc: MockMvc) {
    ...
    fun `getBodyCorporateById should return 200 OK when found`() {
        ...
        mockMvc.perform(get("/api/body-corporates/{id}", testUuid))
    ...
    fun `updateBodyCorporate should return 200 OK on success`() {
        val updateRequest = UpdateBodyCorporateRequest(
    ...
    fun `deleteBodyCorporate should return 200 OK on success`() {
        mockMvc.perform(delete("/api/body-corporates/{id}", testUuid))
            .andExpect(status().isOk)
    ...
    fun `searchBodyCorporatesByName should return 200 OK with results`() {
        ...
        mockMvc.perform(get("/api/body-corporates/search").param("name", "Corp"))
            .andExpect(status().isOk)
