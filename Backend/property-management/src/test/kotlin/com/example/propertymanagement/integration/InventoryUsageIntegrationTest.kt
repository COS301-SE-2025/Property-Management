...
class InventoryUsageIntegrationTest {
    ...
    fun createInventoryItemAndGetUuid(): UUID {
        val buildingDto = BuildingCreateDto(
        ...
        val itemDto = CreateInventoryItemDto(
            name = "TestItem",
            unit = "pcs",
            quantity = 100,
            buildingUuid = buildingUuid
        )
        val itemResponse = restTemplate.postForEntity(
    ...
    fun `should create inventory usage`() {
        ...
        val usageDto = CreateInventoryUsageRequest(
            itemUuid = itemUuid,
            taskUuid = taskUuid,
    ...
    fun `should update inventory usage`() {
        ...
        val usageUuid = postResponse.body!!.usageUuid
        val updateDto = UpdateInventoryUsageRequest(
            quantityUsed = 20,
    ...
    fun `should approve inventory usage`() {
        ...
        val usageUuid = postResponse.body!!.usageUuid
        val approvalDto = ApprovalRequest(
            trusteeApproved = true,
            approvalDate = Date.valueOf(LocalDate.now())
        )
