# Building Endpoints

## Create a Building
**POST** `/api/buildings`

**Request Body Example:**
```json
{
  "name": "Block A",
  "address": "123 Main St",
  "type": "Apartment",
  "propertyValue": 1000000.0,
  "primaryContractors": [1, 2],
  "latestInspectionDate": "2025-06-26",
  "area": 1200.0,
  "trusteeUuid": "e7b8c3a2-9b2e-4d8a-8f6b-123456789abc",
  "coporateUuid": "a1b2c3d4-e5f6-7890-abcd-123456789abc",
  "propertyImageId": "img_123"
}
```

**Response Example (201 Created):**
```json
{
  "name": "Block A",
  "address": "123 Main St",
  "type": "Apartment",
  "propertyValue": 1000000.0,
  "primaryContractors": [1, 2],
  "latestInspectionDate": "2025-06-26",
  "propertyImage": "img_123",
  "area": 1200.0,
  "buildingUuid": "b1c2d3e4-f5a6-7890-bcde-123456789abc",
  "trusteeUuid": "e7b8c3a2-9b2e-4d8a-8f6b-123456789abc",
  "coporateUuid": "a1b2c3d4-e5f6-7890-abcd-123456789abc"
}
```

---

## Get All Buildings
**GET** `/api/buildings`

**Response Example:**
```json
[
  {
    "name": "Block A",
    "address": "123 Main St",
    "type": "Apartment",
    "propertyValue": 1000000.0,
    "primaryContractors": [1, 2],
    "latestInspectionDate": "2025-06-26",
    "propertyImage": "img_123",
    "area": 1200.0,
    "buildingUuid": "b1c2d3e4-f5a6-7890-bcde-123456789abc",
    "trusteeUuid": "e7b8c3a2-9b2e-4d8a-8f6b-123456789abc",
    "coporateUuid": "a1b2c3d4-e5f6-7890-abcd-123456789abc"
  }
]
```

---

## Get Building by UUID
**GET** `/api/buildings/{uuid}`

**Response Example:**
```json
{
  "name": "Block A",
  "address": "123 Main St",
  "type": "Apartment",
  "propertyValue": 1000000.0,
  "primaryContractors": [1, 2],
  "latestInspectionDate": "2025-06-26",
  "propertyImage": "img_123",
  "area": 1200.0,
  "buildingUuid": "b1c2d3e4-f5a6-7890-bcde-123456789abc",
  "trusteeUuid": "e7b8c3a2-9b2e-4d8a-8f6b-123456789abc",
  "coporateUuid": "a1b2c3d4-e5f6-7890-abcd-123456789abc"
}
```

---

## Update Building
**PUT** `/api/buildings/{uuid}`

**Request Body Example:**
```json
{
  "name": "Block A - Updated",
  "address": "123 Main St",
  "type": "Apartment",
  "propertyValue": 1200000.0,
  "primaryContractors": [1, 2, 3],
  "latestInspectionDate": "2025-07-01",
  "area": 1300.0,
  "trusteeUuid": "e7b8c3a2-9b2e-4d8a-8f6b-123456789abc",
  "coporateUuid": "a1b2c3d4-e5f6-7890-abcd-123456789abc",
  "propertyImageId": "img_456"
}
```

**Response Example:**
```json
{
  "name": "Block A - Updated",
  "address": "123 Main St",
  "type": "Apartment",
  "propertyValue": 1200000.0,
  "primaryContractors": [1, 2, 3],
  "latestInspectionDate": "2025-07-01",
  "propertyImage": "img_456",
  "area": 1300.0,
  "buildingUuid": "b1c2d3e4-f5a6-7890-bcde-123456789abc",
  "trusteeUuid": "e7b8c3a2-9b2e-4d8a-8f6b-123456789abc",
  "coporateUuid": "a1b2c3d4-e5f6-7890-abcd-123456789abc"
}
```

---

## Delete Building
**DELETE** `/api/buildings/{uuid}`

**Response:**  
- `204 No Content` if deleted  
- `404 Not Found` if not found

---

## Get Buildings by Trustee
**GET** `/api/buildings/trustee/{trusteeUuid}`

**Response Example:**
```json
{
  "trusteeUuid": "e7b8c3a2-9b2e-4d8a-8f6b-123456789abc",
  "buildings": [
    {
      "name": "Block A",
      "address": "123 Main St",
      "type": "Apartment",
      "propertyValue": 1000000.0,
      "primaryContractors": [1, 2],
      "latestInspectionDate": "2025-06-26",
      "propertyImage": "img_123",
      "area": 1200.0,
      "buildingUuid": "b1c2d3e4-f5a6-7890-bcde-123456789abc",
      "trusteeUuid": "e7b8c3a2-9b2e-4d8a-8f6b-123456789abc",
      "coporateUuid": "a1b2c3d4-e5f6-7890-abcd-123456789abc"
    }
  ]
}
```

---

## Get Buildings by Corporate
**GET** `/api/buildings/corporate/{corporateUuid}`

**Response Example:**
```json
{
  "coporateUuid": "5ad98526-6f94-4ce0-acf9-31e99590f8e2",
  "buildings": [
    {
      "name": "Block A",
      "address": "123 Main St",
      "type": "Apartment",
      "propertyValue": 1000000.0,
      "primaryContractors": [1, 2],
      "latestInspectionDate": "2025-06-26",
      "propertyImage": "img_123",
      "area": 1200.0,
      "buildingUuid": "b1c2d3e4-f5a6-7890-bcde-123456789abc",
      "trusteeUuid": "e7b8c3a2-9b2e-4d8a-8f6b-123456789abc",
      "coporateUuid": "5ad98526-6f94-4ce0-acf9-31e99590f8e2"
    }
  ]
}
```

---

## Search Buildings by Name
**GET** `/api/buildings/search?name={name}`

**Response Example:**
```json
[
  {
    "name": "Block A",
    "address": "123 Main St",
    "type": "Apartment",
    "propertyValue": 1000000.0,
    "primaryContractors": [1, 2],
    "latestInspectionDate": "2025-06-26",
    "propertyImage": "img_123",
    "area": 1200.0,
    "buildingUuid": "b1c2d3e4-f5a6-7890-bcde-123456789abc",
    "trusteeUuid": "e7b8c3a2-9b2e-4d8a-8f6b-123456789abc",
    "coporateUuid": "a1b2c3d4-e5f6-7890-abcd-123456789abc"
  }
]
```

---

## Get Buildings by Type
**GET** `/api/buildings/type/{type}`

**Response Example:**
```json
[
  {
    "name": "Block A",
    "address": "123 Main St",
    "type": "Apartment",
    "propertyValue": 1000000.0,
    "primaryContractors": [1, 2],
    "latestInspectionDate": "2025-06-26",
    "propertyImage": "img_123",
    "area": 1200.0,
    "buildingUuid": "b1c2d3e4-f5a6-7890-bcde-123456789abc",
    "trusteeUuid": "e7b8c3a2-9b2e-4d8a-8f6b-123456789abc",
    "coporateUuid": "a1b2c3d4-e5f6-7890-abcd-123456789abc"
  }
]
```

---

# Inventory Item Endpoints

## Get All Inventory Items
**GET** `/api/inventory`

**Response Example:**
```json
[
  {
    "itemUuid": "b1c2d3e4-f5a6-7890-bcde-123456789abc",
    "name": "Light Bulb",
    "unit": "pieces",
    "quantityInStock": 100,
    "buildingUuidFk": "a1b2c3d4-e5f6-7890-abcd-123456789abc"
  }
]
```

---

## Get Inventory Items by Building
**GET** `/api/inventory/building/{buildingUuid}`

- **Path Variable:**  
  - `buildingUuid` (**required**): UUID of the building

**Response Example:**
```json
[
  {
    "itemUuid": "b1c2d3e4-f5a6-7890-bcde-123456789abc",
    "name": "Light Bulb",
    "unit": "pieces",
    "quantityInStock": 100,
    "buildingUuidFk": "a1b2c3d4-e5f6-7890-abcd-123456789abc"
  }
]
```

---

## Get Inventory Item by UUID
**GET** `/api/inventory/{itemUuid}`

- **Path Variable:**  
  - `itemUuid` (**required**): UUID of the inventory item

**Response Example:**
```json
{
  "itemUuid": "b1c2d3e4-f5a6-7890-bcde-123456789abc",
  "name": "Light Bulb",
  "unit": "pieces",
  "quantityInStock": 100,
  "buildingUuidFk": "a1b2c3d4-e5f6-7890-abcd-123456789abc"
}
```

---

## Create Inventory Item
**POST** `/api/inventory`

**Request Body Fields:**
- `name` (**required**): Name of the inventory item (string)
- `unit` (**required**): Unit of measurement (string)
- `quantity` (**required**): Initial quantity (integer, zero or positive)
- `buildingUuid` (**required**): UUID of the building (string/UUID)

**Request Example:**
```json
{
  "name": "Pipe Segment",
  "unit": "meters",
  "quantity": 50,
  "buildingUuid": "a1b2c3d4-e5f6-7890-abcd-123456789abc"
}
```

**Response Example (201 Created):**
```json
{
  "itemUuid": "b1c2d3e4-f5a6-7890-bcde-123456789abc",
  "name": "Pipe Segment",
  "unit": "meters",
  "quantityInStock": 50,
  "buildingUuidFk": "a1b2c3d4-e5f6-7890-abcd-123456789abc"
}
```

---

## Update Inventory Item
**PUT** `/api/inventory/{itemUuid}`

- **Path Variable:**  
  - `itemUuid` (**required**): UUID of the inventory item

**Request Body Fields:**  
_All fields are optional; only include the ones you want to update._
- `name` (string, optional)
- `unit` (string, optional)
- `quantity` (integer, optional, must be zero or positive)

**Request Example:**
```json
{
  "name": "LED Lightbulb",
  "quantity": 75
}
```

**Response Example:**
```json
{
  "itemUuid": "b1c2d3e4-f5a6-7890-bcde-123456789abc",
  "name": "LED Lightbulb",
  "unit": "pieces",
  "quantityInStock": 75,
  "buildingUuidFk": "a1b2c3d4-e5f6-7890-abcd-123456789abc"
}
```

---

## Update Inventory Quantity (Add, Subtract, or Set)
**PATCH** `/api/inventory/{itemUuid}/quantity`

- **Path Variable:**  
  - `itemUuid` (**required**): UUID of the inventory item

**Request Body Fields:**
- `quantity` (**required**): The quantity to set/add/subtract (integer)
- `operation` (**required**): `"SET"`, `"ADD"`, or `"SUBTRACT"` (string, case-insensitive)

**Request Example (add 10):**
```json
{
  "quantity": 10,
  "operation": "ADD"
}
```

**Request Example (set to 50):**
```json
{
  "quantity": 50,
  "operation": "SET"
}
```

**Response Example:**
```json
{
  "itemUuid": "b1c2d3e4-f5a6-7890-bcde-123456789abc",
  "name": "Pipe Segment",
  "unit": "meters",
  "quantityInStock": 60,
  "buildingUuidFk": "a1b2c3d4-e5f6-7890-abcd-123456789abc"
}
```

---

## Delete Inventory Item
**DELETE** `/api/inventory/{itemUuid}`

- **Path Variable:**  
  - `itemUuid` (**required**): UUID of the inventory item

**Response:**  
- `204 No Content` if deleted  
- `404 Not Found` if not found

---

# Inventory Usage Endpoints

## Create Inventory Usage
**POST** `/api/inventory-usage`

**Request Body Fields:**
- `itemUuid` (**required**): UUID of the inventory item
- `taskUuid` (**required**): UUID of the related task
- `usedByContractorUuid` (**required**): UUID of the contractor who used the item
- `quantityUsed` (**required**): Number of items used (integer)

**Request Example:**
```json
{
  "itemUuid": "b1c2d3e4-f5a6-7890-bcde-123456789abc",
  "taskUuid": "c2d3e4f5-a6b7-8901-cdef-23456789abcd",
  "usedByContractorUuid": "d3e4f5a6-b7c8-9012-def3-3456789abcde",
  "quantityUsed": 5
}
```

**Response Example (201 Created):**
```json
{
  "usageUuid": "e4f5a6b7-c8d9-0123-ef45-456789abcdef",
  "itemUuid": "b1c2d3e4-f5a6-7890-bcde-123456789abc",
  "taskUuid": "c2d3e4f5-a6b7-8901-cdef-23456789abcd",
  "usedByContractorUuid": "d3e4f5a6-b7c8-9012-def3-3456789abcde",
  "quantityUsed": 5,
  "trusteeApproved": false,
  "approvalDate": null
}
```

---

## Get All Inventory Usage (Paged)
**GET** `/api/inventory-usage?page=0&size=10&sortBy=usageUuid&sortDir=asc`

**Query Parameters:**
- `page` (optional, default: 0)
- `size` (optional, default: 10)
- `sortBy` (optional, default: "usageUuid")
- `sortDir` (optional, default: "asc")

**Response Example:**
```json
{
  "content": [
    {
      "usageUuid": "e4f5a6b7-c8d9-0123-ef45-456789abcdef",
      "itemUuid": "b1c2d3e4-f5a6-7890-bcde-123456789abc",
      "taskUuid": "c2d3e4f5-a6b7-8901-cdef-23456789abcd",
      "usedByContractorUuid": "d3e4f5a6-b7c8-9012-def3-3456789abcde",
      "quantityUsed": 5,
      "trusteeApproved": false,
      "approvalDate": null
    }
  ],
  "totalElements": 1,
  "totalPages": 1,
  "size": 10,
  "number": 0
}
```

---

## Get Inventory Usage by UUID
**GET** `/api/inventory-usage/{usageUuid}`

- **Path Variable:**  
  - `usageUuid` (**required**): UUID of the usage record

**Response Example:**
```json
{
  "usageUuid": "e4f5a6b7-c8d9-0123-ef45-456789abcdef",
  "itemUuid": "b1c2d3e4-f5a6-7890-bcde-123456789abc",
  "taskUuid": "c2d3e4f5-a6b7-8901-cdef-23456789abcd",
  "usedByContractorUuid": "d3e4f5a6-b7c8-9012-def3-3456789abcde",
  "quantityUsed": 5,
  "trusteeApproved": false,
  "approvalDate": null
}
```

---

## Update Inventory Usage
**PUT** `/api/inventory-usage/{usageUuid}`

- **Path Variable:**  
  - `usageUuid` (**required**): UUID of the usage record

**Request Body Fields:**  
_All fields are optional; only include the ones you want to update._
- `quantityUsed` (integer, optional)
- `trusteeApproved` (boolean, optional)
- `approvalDate` (date, optional, format: "YYYY-MM-DD")

**Request Example:**
```json
{
  "quantityUsed": 10,
  "trusteeApproved": true,
  "approvalDate": "2025-06-26"
}
```

**Response Example:**
```json
{
  "usageUuid": "e4f5a6b7-c8d9-0123-ef45-456789abcdef",
  "itemUuid": "b1c2d3e4-f5a6-7890-bcde-123456789abc",
  "taskUuid": "c2d3e4f5-a6b7-8901-cdef-23456789abcd",
  "usedByContractorUuid": "d3e4f5a6-b7c8-9012-def3-3456789abcde", // can be null 
  "quantityUsed": 10,
  "trusteeApproved": true,
  "approvalDate": "2025-06-26"
}
```

---

## Delete Inventory Usage
**DELETE** `/api/inventory-usage/{usageUuid}`

- **Path Variable:**  
  - `usageUuid` (**required**): UUID of the usage record

**Response Example (200 OK):**
```json
{ "message": "Inventory usage deleted successfully" }
```
- `404 Not Found` if not found

---

## Assign contractor 
**PATCH** `/api/inventory-usage/{usageUuid}/assign-contractor`

- **Path Variable:**  
  - `usageUuid` (**required**): UUID of the usage record

**Request Body Fields:**
- `contractorUuid` (**required**): UUID of contractor you want to assign to

**Response Example (200 OK):**
```json
{ "message": "Inventory usage deleted successfully" }
```
- `404 Not Found` if not found

---

## Approve Inventory Usage
**PATCH** `/api/inventory-usage/{usageUuid}/approval`

- **Path Variable:**  
  - `usageUuid` (**required**): UUID of the usage record

**Request Body Fields:**
- `trusteeApproved` (**required**): boolean
- `approvalDate` (optional, date, default: now)

**Request Example:**
```json
{
  "trusteeApproved": true,
  "approvalDate": "2025-06-26"
}
```

**Response Example:**
```json
{
  "usageUuid": "e4f5a6b7-c8d9-0123-ef45-456789abcdef",
  "itemUuid": "b1c2d3e4-f5a6-7890-bcde-123456789abc",
  "taskUuid": "c2d3e4f5-a6b7-8901-cdef-23456789abcd",
  "usedByContractorUuid": "d3e4f5a6-b7c8-9012-def3-3456789abcde",
  "quantityUsed": 10,
  "trusteeApproved": true,
  "approvalDate": "2025-06-26"
}
```

---

## Get Usage by Item
**GET** `/api/inventory-usage/by-item/{itemUuid}`

- **Path Variable:**  
  - `itemUuid` (**required**): UUID of the inventory item

**Response Example:**
```json
[
  {
    "usageUuid": "e4f5a6b7-c8d9-0123-ef45-456789abcdef",
    "itemUuid": "b1c2d3e4-f5a6-7890-bcde-123456789abc",
    "taskUuid": "c2d3e4f5-a6b7-8901-cdef-23456789abcd",
    "usedByContractorUuid": "d3e4f5a6-b7c8-9012-def3-3456789abcde",
    "quantityUsed": 5,
    "trusteeApproved": false,
    "approvalDate": null
  }
]
```

---

## Get Usage by Task
**GET** `/api/inventory-usage/by-task/{taskUuid}`

- **Path Variable:**  
  - `taskUuid` (**required**): UUID of the task

**Response Example:** _Same as above, but filtered by taskUuid_

---

## Get Usage by Contractor
**GET** `/api/inventory-usage/by-contractor/{contractorUuid}`

- **Path Variable:**  
  - `contractorUuid` (**required**): UUID of the contractor

**Response Example:** _Same as above, but filtered by contractorUuid_

---

## Get Approved Usage
**GET** `/api/inventory-usage/approved`

**Response Example:**
```json
[
  {
    "usageUuid": "e4f5a6b7-c8d9-0123-ef45-456789abcdef",
    "itemUuid": "b1c2d3e4-f5a6-7890-bcde-123456789abc",
    "taskUuid": "c2d3e4f5-a6b7-8901-cdef-23456789abcd",
    "usedByContractorUuid": "d3e4f5a6-b7c8-9012-def3-3456789abcde",
    "quantityUsed": 5,
    "trusteeApproved": true,
    "approvalDate": "2025-06-26"
  }
]
```

---

## Get Pending Approval Usage
**GET** `/api/inventory-usage/pending-approval`

**Response Example:** _Same as above, but only with `trusteeApproved: false`_

---

## Get Total Quantity Used for Item
**GET** `/api/inventory-usage/total-quantity/item/{itemUuid}`

- **Path Variable:**  
  - `itemUuid` (**required**): UUID of the inventory item

**Response Example:**
```json
{ "totalQuantityUsed": 25 }
```

---

## Get Total Quantity Used by Contractor
**GET** `/api/inventory-usage/total-quantity/contractor/{contractorUuid}`

- **Path Variable:**  
  - `contractorUuid` (**required**): UUID of the contractor

**Response Example:**
```json
{ "totalQuantityUsed": 12 }
```

---

# Budget Endpoints

## Create Budget
**POST** `/api/budgets`

**Request Body Fields:**
- `year` (**required**): Budget year (integer)
- `totalBudget` (**required**): Total budget (decimal)
- `maintenanceBudget` (**required**): Maintenance budget (decimal)
- `inventoryBudget` (**required**): Inventory budget (decimal)
- `approvalDate` (**required**): Approval date (string, format: "YYYY-MM-DD")
- `notes` (optional): Notes (string)
- `buildingUuid` (**required**): UUID of the building

**Request Example:**
```json
{
  "year": 2025,
  "totalBudget": 100000.00,
  "maintenanceBudget": 30000.00,
  "inventoryBudget": 20000.00,
  "approvalDate": "2025-06-26",
  "notes": "Annual budget for Block A",
  "buildingUuid": "a1b2c3d4-e5f6-7890-abcd-123456789abc"
}
```

**Response Example (201 Created):**
```json
{
  "budgetUuid": "b1c2d3e4-f5a6-7890-bcde-123456789abc",
  "year": 2025,
  "totalBudget": 100000.00,
  "maintenanceBudget": 30000.00,
  "inventoryBudget": 20000.00,
  "approvalDate": "2025-06-26",
  "notes": "Annual budget for Block A",
  "inventorySpent": 0.00,
  "maintenanceSpent": 0.00,
  "buildingUuid": "a1b2c3d4-e5f6-7890-abcd-123456789abc"
}
```

---

## Get Budget by UUID
**GET** `/api/budgets/{budgetUuid}`

- **Path Variable:**  
  - `budgetUuid` (**required**): UUID of the budget

**Response Example:**
```json
{
  "budgetUuid": "b1c2d3e4-f5a6-7890-bcde-123456789abc",
  "year": 2025,
  "totalBudget": 100000.00,
  "maintenanceBudget": 30000.00,
  "inventoryBudget": 20000.00,
  "approvalDate": "2025-06-26",
  "notes": "Annual budget for Block A",
  "inventorySpent": 0.00,
  "maintenanceSpent": 0.00,
  "buildingUuid": "a1b2c3d4-e5f6-7890-abcd-123456789abc"
}
```

---

## Get All Budgets
**GET** `/api/budgets`

**Response Example:**
```json
[
  {
    "budgetUuid": "b1c2d3e4-f5a6-7890-bcde-123456789abc",
    "year": 2025,
    "totalBudget": 100000.00,
    "maintenanceBudget": 30000.00,
    "inventoryBudget": 20000.00,
    "approvalDate": "2025-06-26",
    "notes": "Annual budget for Block A",
    "inventorySpent": 0.00,
    "maintenanceSpent": 0.00,
    "buildingUuid": "a1b2c3d4-e5f6-7890-abcd-123456789abc"
  }
]
```

---

## Get Budgets by Building UUID
**GET** `/api/budgets/building/{buildingUuid}`

- **Path Variable:**  
  - `buildingUuid` (**required**): UUID of the building

**Response Example:** _Same as above, but filtered by buildingUuid_

---

## Get Budgets by Year
**GET** `/api/budgets/year/{year}`

- **Path Variable:**  
  - `year` (**required**): Year (integer)

**Response Example:** _Same as above, but filtered by year_

---

## Get Budget by Building and Year
**GET** `/api/budgets/building/{buildingUuid}/year/{year}`

- **Path Variables:**  
  - `buildingUuid` (**required**): UUID of the building  
  - `year` (**required**): Year (integer)

**Response Example:**
```json
{
  "budgetUuid": "b1c2d3e4-f5a6-7890-bcde-123456789abc",
  "year": 2025,
  "totalBudget": 100000.00,
  "maintenanceBudget": 30000.00,
  "inventoryBudget": 20000.00,
  "approvalDate": "2025-06-26",
  "notes": "Annual budget for Block A",
  "inventorySpent": 0.00,
  "maintenanceSpent": 0.00,
  "buildingUuid": "a1b2c3d4-e5f6-7890-abcd-123456789abc"
}
```

---

## Update Budget
**PUT** `/api/budgets/{budgetUuid}`

- **Path Variable:**  
  - `budgetUuid` (**required**): UUID of the budget

**Request Body Fields:**  
_All fields are optional; only include the ones you want to update._
- `year` (integer, optional)
- `totalBudget` (decimal, optional)
- `maintenanceBudget` (decimal, optional)
- `inventoryBudget` (decimal, optional)
- `approvalDate` (string, optional, format: "YYYY-MM-DD")
- `notes` (string, optional)
- `inventorySpent` (decimal, optional)
- `maintenanceSpent` (decimal, optional)
- `buildingUuid` (UUID, optional)

**Request Example:**
```json
{
  "year": 2026,
  "totalBudget": 120000.00,
  "maintenanceBudget": 35000.00,
  "notes": "Updated budget"
}
```

**Response Example:**
```json
{
  "budgetUuid": "b1c2d3e4-f5a6-7890-bcde-123456789abc",
  "year": 2026,
  "totalBudget": 120000.00,
  "maintenanceBudget": 35000.00,
  "inventoryBudget": 20000.00,
  "approvalDate": "2025-06-26",
  "notes": "Updated budget",
  "inventorySpent": 0.00,
  "maintenanceSpent": 0.00,
  "buildingUuid": "a1b2c3d4-e5f6-7890-abcd-123456789abc"
}
```

---

## Delete Budget
**DELETE** `/api/budgets/{budgetUuid}`

- **Path Variable:**  
  - `budgetUuid` (**required**): UUID of the budget

**Response:**  
- `204 No Content` if deleted  
- `404 Not Found` if not found

---

# Body Corporate Endpoints

## Register Body Corporate
**POST** `/api/body-corporates/register`

**Request Body Fields:**
- `corporateName` (**required**): Name of the body corporate (string)
- `contributionPerSqm` (**required**): Contribution per square meter (decimal, > 0)
- `totalBudget` (optional, default: 0): Total budget (decimal, non-negative)
- `email` (**required**): Email address (string, valid email)
- `password` (**required**): Password (string, min 8 characters)
- `contactNumber` (optional): Contact number (string)

**Request Example:**
```json
{
  "corporateName": "Sunset Villas",
  "contributionPerSqm": 12.50,
  "totalBudget": 50000.00,
  "email": "admin@sunsetvillas.com",
  "password": "securePass123",
  "contactNumber": "0123456789"
}
```

**Response Example (201 Created):**
```json
{
  "corporateUuid": "c1d2e3f4-5678-90ab-cdef-123456789abc",
  "corporateName": "Sunset Villas",
  "email": "admin@sunsetvillas.com",
  "cognitoUserId": "cognito-uuid-123",
  "username": "admin_1719420000000",
  "emailVerificationRequired": true
}
```

---

## Confirm Registration
**POST** `/api/body-corporates/confirm-registration`

**Request Body Fields:**
- `username` (**required**): Username used for registration (string)
- `code` (**required**): Confirmation code sent to email (string)

**Request Example:**
```json
{
  "username": "admin_1719420000000",
  "code": "123456"
}
```

**Response Example (200 OK):**
```json
{
  "message": "Registration confirmed successfully"
}
```

---

## Login
**POST** `/api/body-corporates/login`

**Request Body Fields:**
- `email` (**required**): Email address (string)
- `password` (**required**): Password (string)

**Request Example:**
```json
{
  "email": "admin@sunsetvillas.com",
  "password": "securePass123"
}
```

**Response Example (200 OK):**
```json
{
  "idToken": "eyJraWQiOiJr...",
  "accessToken": "eyJraWQiOiJr...",
  "refreshToken": "eyJraWQiOiJr...",
  "userType": "BODY_CORPORATE",
  "userId": "cognito-uuid-123"
}
```

---

## Get All Body Corporates (Paged)
**GET** `/api/body-corporates?page=0&size=20&sortBy=corporateName&sortDirection=asc`

**Query Parameters:**
- `page` (optional, default: 0)
- `size` (optional, default: 20)
- `sortBy` (optional, default: "corporateName")
- `sortDirection` (optional, default: "asc")

**Response Example:**
```json
{
  "content": [
    {
      "corporateUuid": "c1d2e3f4-5678-90ab-cdef-123456789abc",
      "corporateName": "Sunset Villas",
      "contributionPerSqm": 12.50,
      "totalBudget": 50000.00,
      "email": "admin@sunsetvillas.com",
      "userId": "cognito-uuid-123",
      "username": "admin_1719420000000"
    }
  ],
  "totalElements": 1,
  "totalPages": 1,
  "size": 20,
  "number": 0
}
```

---

## Get Body Corporate by UUID
**GET** `/api/body-corporates/{id}`

- **Path Variable:**  
  - `id` (**required**): UUID of the body corporate

**Response Example:**
```json
{
  "corporateUuid": "c1d2e3f4-5678-90ab-cdef-123456789abc",
  "corporateName": "Sunset Villas",
  "contributionPerSqm": 12.50,
  "totalBudget": 50000.00,
  "email": "admin@sunsetvillas.com",
  "userId": "cognito-uuid-123",
  "username": "admin_1719420000000"
}
```

---

## Get Body Corporate by Email
**GET** `/api/body-corporates/email/{email}`

- **Path Variable:**  
  - `email` (**required**): Email address

**Response Example:** _Same as above_

---

## Get Body Corporate by User ID
**GET** `/api/body-corporates/user/{userId}`

- **Path Variable:**  
  - `userId` (**required**): Cognito user ID

**Response Example:** _Same as above_

---

## Update Body Corporate
**PUT** `/api/body-corporates/{id}`

- **Path Variable:**  
  - `id` (**required**): UUID of the body corporate

**Request Body Fields:**  
_All fields are optional; only include the ones you want to update._
- `corporateName` (string, optional)
- `contributionPerSqm` (decimal, optional)
- `totalBudget` (decimal, optional)
- `email` (string, optional)

**Request Example:**
```json
{
  "corporateName": "Sunset Villas Updated",
  "totalBudget": 60000.00
}
```

**Response Example:**
```json
{
  "corporateUuid": "c1d2e3f4-5678-90ab-cdef-123456789abc",
  "corporateName": "Sunset Villas Updated",
  "contributionPerSqm": 12.50,
  "totalBudget": 60000.00,
  "email": "admin@sunsetvillas.com",
  "userId": "cognito-uuid-123",
  "username": "admin_1719420000000"
}
```

---

## Delete Body Corporate
**DELETE** `/api/body-corporates/{id}`

- **Path Variable:**  
  - `id` (**required**): UUID of the body corporate

**Response Example (200 OK):**
```json
{ "message": "Body corporate deleted successfully" }
```
- `404 Not Found` if not found

---

## Search Body Corporates by Name
**GET** `/api/body-corporates/search?name={name}`

**Response Example:**
```json
[
  {
    "corporateUuid": "c1d2e3f4-5678-90ab-cdef-123456789abc",
    "corporateName": "Sunset Villas",
    "contributionPerSqm": 12.50,
    "totalBudget": 50000.00,
    "email": "admin@sunsetvillas.com",
    "userId": "cognito-uuid-123",
    "username": "admin_1719420000000"
  }
]
```

---

## Filter Body Corporates by Contribution Range
**GET** `/api/body-corporates/filter/contribution?minContribution=10&maxContribution=20`

**Query Parameters:**
- `minContribution` (**required**): Minimum contribution per sqm (decimal)
- `maxContribution` (**required**): Maximum contribution per sqm (decimal)

**Response Example:** _Same as above, but filtered by contribution range_

---

## Filter Body Corporates by Minimum Budget
**GET** `/api/body-corporates/filter/budget?minBudget=40000`

**Query Parameters:**
- `minBudget` (**required**): Minimum total budget (decimal)

**Response Example:** _Same as above, but filtered by minimum budget_

---

## Get Body Corporate Statistics
**GET** `/api/body-corporates/statistics`

**Response Example:**
```json
{
  "totalBodyCorporates": 5,
  "totalCombinedBudget": 250000.00
}
```

---

### Field Requirements Summary

| Field              | Create | Update | Type      | Notes                        |
|--------------------|--------|--------|-----------|------------------------------|
| corporateName      | req    | opt    | String    | Name of the body corporate   |
| contributionPerSqm | req    | opt    | Decimal   | Contribution per sqm (> 0)   |
| totalBudget        | opt    | opt    | Decimal   | Non-negative, default 0      |
| email              | req    | opt    | String    | Valid email                  |
| password           | req    | -      | String    | Min 8 characters             |
| contactNumber      | opt    | -      | String    | Optional                     |

- **req** = required, **opt** = optional, **-** = not used

---

# Contractor Endpoints

## Create a Contractor
**POST** `/api/contractor`

**Request Body Example:**
```json
{
  "name": "Karabelo",
  "contact_info": "Test",
  "status": false,
  "apikey": "0210323-2313-123",
  "email": "karabelotaole04@gmail.com",
  "phone": "013456789",
  "address": "Tes street",
  "city": "Pretoria",
  "postal_code": "0419",
  "reg_number": "23123123131",
  "description": "Good pipe layer",
  "services": "Pipe laying",
  "img": {UUID}
}
img refering to the images table for the uuid

```

**Response Example (201 Created):**
```json
{
  "uuid": "d368aef5-0f2d-46a4-b32e-1e2b79cc7e28",
  "name": "Karabelo",
  "contact_info": "Test",
  "status": false,
  "apikey": "0210323-2313-123",
  "email": "karabelotaole04@gmail.com",
  "phone": "013456789",
  "address": "Tes street",
  "city": "Pretoria",
  "postal_code": "0419",
  "reg_number": "23123123131",
  "description": "Good pipe layer",
  "services": "Pipe laying",
  "img": "d368aef5-0f2d-46a4-b32e-1e2b79cc7e28"
}

img refering to the images table for the uuid
```

---

## Get All Contractors
**GET** `/api/contractor`

**Response Example:**
```json
[
  {
    "uuid": "d368aef5-0f2d-46a4-b32e-1e2b79cc7e28",
    "name": "Karabelo",
    "contact_info": "Test",
    "status": false,
    "apikey": "0210323-2313-123",
    "email": "karabelotaole04@gmail.com",
    "phone": "013456789",
    "address": "Tes street",
    "city": "Pretoria",
    "postal_code": "0419",
    "reg_number": "23123123131",
    "description": "Good pipe layer",
    "services": "Pipe laying",
    "img": "d368aef5-0f2d-46a4-b32e-1e2b79cc7e28",
  }
]
```

---

## Get Contractor by UUID
**GET** `/api/contractor/{uuid}`

- **Path Variable:**  
  - `uuid` (**required**): UUID of the contractor

**Response Example:**
```json
{
  "uuid": "d368aef5-0f2d-46a4-b32e-1e2b79cc7e28",
  "name": "Karabelo",
  "contact_info": "Test",
  "status": false,
  "apikey": "0210323-2313-123",
  "email": "karabelotaole04@gmail.com",
  "phone": "013456789",
  "address": "Tes street",
  "city": "Pretoria",
  "postal_code": "0419",
  "reg_number": "23123123131",
  "description": "Good pipe layer",
  "services": "Pipe laying",
  "img": "d368aef5-0f2d-46a4-b32e-1e2b79cc7e28",
}

# Contractor Authentication Endpoints

## Register a Contractor
**POST** `/api/contractor/auth/register`

**Request Body Example:**
```json
{
  "email": "bagoki1587@asimarif.com",
  "password": "Test@1234",
  "contactNumber": "0672687235"
}
```

**Response Example (200 OK):**
```json
{
  "message": "Registration successful, please verify your email.",
  "username": "bagoki15871750943941252"
}
```

> **Note:** Store the username for the next step (email confirmation).

---

## Confirm Email
**POST** `/api/contractor/auth/confirm`

**Request Body Example:**
```json
{
  "username": "bagoki15871750941249379",
  "code": "621074"
}
```

**Response Example:**
```json
{
  "message": "Account confirmed."
}
```

---

## Login
**POST** `/api/contractor/auth/login`

**Request Body Example:**
```json
{
  "email": "bagoki1587@asimarif.com",
  "password": "Test@1234"
}
```

**Response Example (200 OK):**
```json
{
  "idToken": "[some long token]",
  "accessToken": "[some long token]",
  "refreshToken": "[some long token]",
  "userType": "contractor",
  "userId": "5b029df7-1e95-439e-89d9-bb2bdc4a3766"
}
```

---

## Update Contractor
**PUT** `/api/contractor`

**Request Body Example:**
```json
{
  "contact_info": "Test Contact – Available via phone and email",
  "status": true,
  "apikey": "0210323-2313-123",
  "email": "karabelo.moeketsi@example.com",
  "phone": "01232313 - New Updated"
}
```

**Response Example:**
```json
{
  "uuid": "d368aef5-0f2d-46a4-b32e-1e2b79cc7e28",
  "name": "Karabelo",
  "contact_info": "Test Contact – Available via phone and email",
  "status": true,
  "apikey": "0210323-2313-123",
  "email": "karabelo.moeketsi@example.com",
  "phone": "01232313 - New Updated",
  "address": "Tes street",
  "city": "Pretoria",
  "postal_code": "0419",
  "reg_number": "23123123131",
  "description": "Good pipe layer",
  "services": "Pipe laying",
  "img": "d368aef5-0f2d-46a4-b32e-1e2b79cc7e28",
}
```

---

## Delete Contractor
**DELETE** `/api/contractor/{uuid}`

- **Path Variable:**  
  - `uuid` (**required**): UUID of the contractor

**Response:**  
- `204 No Content` if deleted  
- `404 Not Found` if not found



# Image Upload Endpoints

## Upload Image
**POST** `/api/images/upload`

**Content-Type:** `multipart/form-data`

**Description:** Uploads a single image file to S3 and returns a unique image key (usually a UUID).

**Request Example:**
```
Content-Type: multipart/form-data
[File upload field with image data]
```

**Response Example (201 Created):**
```json
{
  "imageKey": "a3f07d2b-2a31-4c9e-bf9e-6f2eaa2c1234"
}
```

**Usage Notes:**
- The returned `imageKey` can be used in other endpoints where image references are needed
- Supported image formats: JPEG, PNG, GIF, WebP
- Maximum file size: 10MB

---

## Get Image URL
**GET** `/api/images/presigned/{uuid}`

- **Path Variable:**  
  - `uuid` (**required**): UUID of the image (imageKey from upload)

**Description:** Returns a presigned URL that can be used to view the image directly.

**Response Example:**
```json
{
  "url": "https://s3.amazonaws.com/bucket-name/images/a3f07d2b-2a31-4c9e-bf9e-6f2eaa2c1234?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=..."
}
```

**Usage Notes:**
- The returned URL is temporary and will expire after a set time period
- Use this URL to display images in your application
- The URL can be used directly in `<img>` tags or API responses

---

### Quote Field Requirements Summary

| Field         | Create | Update | Type      | Notes                           |
|---------------|--------|--------|-----------|--------------------------------|
| task_id       | req*   | opt    | String    | Required for Maintenance quotes |
| contractor_id | req*   | opt    | String    | Required for Maintenance quotes |
| amount        | req    | opt    | Decimal   | Quote amount                   |
| submitted_on  | req    | opt    | Timestamp | ISO 8601 format               |
| type          | req    | opt    | String    | "Maintenance" or "Inventory"   |
| expiry_date   | req    | opt    | Timestamp | ISO 8601 format               |

- **req** = required, **opt** = optional, **req*** = required for Maintenance quotes only

### Contractor Field Requirements Summary

| Field        | Create | Update | Type    | Notes                    |
|--------------|--------|--------|---------|--------------------------|
| name         | req    | opt    | String  | Contractor name          |
| contact_info | req    | opt    | String  | Contact information      |
| status       | req    | opt    | Boolean | Active/inactive status   |
| apikey       | req    | opt    | String  | API key for contractor   |
| email        | req    | opt    | String  | Email address            |
| phone        | req    | opt    | String  | Phone number             |
| address      | req    | opt    | String  | Physical address         |
| city         | req    | opt    | String  | City                     |
| postal_code  | req    | opt    | String  | Postal code              |
| reg_number   | req    | opt    | String  | Registration number      |
| description  | req    | opt    | String  | Contractor description   |
| services     | req    | opt    | String  | Services offered         |

- **req** = required, **opt** = optional

# Maintenance Task Endpoints

## Create Maintenance Task
**POST** `/api/maintenance/create`

**Headers:**
- **Key**`isOWner`: **value** boolean (true/false)
- **Key**`isBodyCorporate`: **value** boolean (true/false)

**Request Body Fields:**
- `title` (**required**): Title of the maintenance task (string)
- `description` (**required**): Description of the maintenance task (string)
- `scheduled_date` (**required**): Scheduled date for the task (string, format: "YYYY-MM-DD")
- `buildingUuid` (**required**): UUID of the building (string/UUID)
- `imageUuid` (**required**): UUID of the image from images table (string/UUID)
- `trusteeUuid` (**required**): UUID of the trustee who created the task (string/UUID)

**Request Body Example:**
```json
{
    "title": "Repair Elevator",
    "description": "Fix elevator stuck on 3rd floor",
    "scheduledDate": "2025-08-10",
    "buildingUuid": "09a1909e-ff8b-4361-a737-c54141b141cf",
    "createdByUuid": "4d33a2ce-110d-403a-9172-73baca2e5664",
    "imageUuid": "bdab0887-4d84-4799-b862-fcb6a1a927cc",
    "trusteeUuid": "4d33a2ce-110d-403a-9172-73baca2e5664"
}

```
## Get All Maintenance Tasks
**GET** `/api/maintenance`

**Description:** Returns all maintenance tasks in the system.

**Response Example:**
```json
[
  {
    "uuid": "d91657fd-98e6-41d1-8afa-7773a53ed915",
    "title": "Yes b yes",
    "des": "djewoijdwoei",
    "status": "cascsac",
    "scheduled_date": "2025-06-18",
    "approved": false,
    "b_uuid": "67b2a69a-7823-474f-ad68-64aee3c94cc4",
    "img": "80666f00-37a0-4749-9631-dc6befda031c",
    "t_uuid": "c375bc26-d60f-403a-a62c-7c888296da6d"
  }
]
```

---

## Get Maintenance Task by UUID
**GET** `/api/maintenance/{uuid}`

- **Path Variable:**  
  - `uuid` (**required**): UUID of the maintenance task

**Description:** Returns a specific maintenance task by its UUID.

**Response Example:**
```json
{
  "uuid": "d91657fd-98e6-41d1-8afa-7773a53ed915",
  "title": "Yes b yes",
  "des": "djewoijdwoei",
  "status": "cascsac",
  "scheduled_date": "2025-06-18",
  "approved": false,
  "b_uuid": "67b2a69a-7823-474f-ad68-64aee3c94cc4",
  "img": "80666f00-37a0-4749-9631-dc6befda031c",
  "t_uuid": "c375bc26-d60f-403a-a62c-7c888296da6d"
}
```

---

## Update Maintenance Task
**PUT** `/api/maintenance/update/{uuid}`

- **Path Variable:**  
  - `uuid` (**required**): UUID of the maintenance task

**Request Body Fields:**  
_All fields are optional; only include the ones you want to update._
- `title` (string, optional)
- `des` (string, optional)
- `status` (string, optional)
- `scheduled_date` (string, optional, format: "YYYY-MM-DD")
- `approved` (boolean, optional)
- `b_uuid` (string/UUID, optional)
- `img` (string/UUID, optional)
- `t_uuid` (string/UUID, optional)

**Request Example:**
```json
{
  "title": "Updated Task Title",
  "status": "In Progress",
  "approved": true
}
```

**Response Example:**
```json
{
  "uuid": "d91657fd-98e6-41d1-8afa-7773a53ed915",
  "title": "Updated Task Title",
  "des": "djewoijdwoei",
  "status": "In Progress",
  "scheduled_date": "2025-06-18",
  "approved": true,
  "b_uuid": "67b2a69a-7823-474f-ad68-64aee3c94cc4",
  "img": "80666f00-37a0-4749-9631-dc6befda031c",
  "t_uuid": "c375bc26-d60f-403a-a62c-7c888296da6d"
}
```

---

## update task(approve)
**PUT** `/api/maintenance/update/{uuid}`

- **Path Variable:**  
  - `uuid` (**required**): UUID of the maintenance task

**Request Body Fields:**  
- `approvalStatus`: (String)

**Request Example:**
```json
{
  "approvalStatus": "APPROVED"
}
```

---

## Assign Contractors
**POST** `/api/maintenance/assign-contractors`

**Description** Assign contractors to task.

**Request Body Fields:**  
- `taskUuid` (**required**) : UUID of the maintenance task
- `contractorUuids` (**required**) : comma separated array of contractor uuids

**Response Example:**
```json
{
    "taskUuid": "5c3588ce-20df-4cfe-baa3-39355e398f56",
    "contractorUuids": [
    "288b32c8-6123-497c-bf7e-e8fa99a075a8",
    "098f75f5-0f57-4a5d-82d1-d5e881e15fa7"
  ]
}
```
---

## submit quote
**POST** `/api/maintenance/quotes`

**Description** submit a qoute.

**Request Header:**
- **key** `isBodyCorporate`, **value** `true` ,**type** `text`

**Response Example:**
```json
{
    "taskUuid": "5c3588ce-20df-4cfe-baa3-39355e398f56",
    "contractorUuid":"288b32c8-6123-497c-bf7e-e8fa99a075a8",
    "amount" : 5000.00,
    "submittedOn": "2025-07-25T15:24:00.000Z",
    "status": "SUBMITTED",
    "documentUrl": "http://example.com/quote1.pdf",
    "expiryDate": "2025-08-25T15:24:00.000Z",
}
```
---

## Delete Maintenance Task
**DELETE** `/api/maintenance/{uuid}`

- **Path Variable:**  
  - `uuid` (**required**): UUID of the maintenance task

**Description:** Deletes a specific maintenance task.

**Response:**  
- `204 No Content` if deleted  
- `404 Not Found` if not found

---

## Get Assigned Contractors

**Method:** `GET`  
**URL:** `/api/maintenance/task/{taskUuid}/contractors`

**Description:**  
Returns the list of contractors assigned to a specific maintenance task.

**Request Parameters:**  
- `taskUuid` – UUID of the maintenance task (in URL path)

**Example Request:**  
```http
GET /api/maintenance/5c3588ce-20df-4cfe-baa3-39355e398f56/contractors 
```

**Response:**  
```json
[
  {
    "contractorUuid": "abc123...",
    "name": "FixIt Contractors",
    "quoteSubmitted": true
  },
  ...
]
```

---

## Get Contractor Maintenance Tasks

**Method:** `GET`  
**URL:** `/api/maintenance/contractor/{contractorUuid}`

**Headers:**  
- `isBodyCorporate: true`

**Request Body:**  
```json
{
  "taskUuid": "5c3588ce-20df-4cfe-baa3-39355e398f56"
}
```

**Description:**  
Fetches all maintenance tasks assigned to a specific contractor. Intended for verifying which tasks a contractor can view and access.

**Example Request:**  
```http
GET /api/maintenance/contractor/288b32c8-6123-497c-bf7e-e8fa99a075a8
Host: localhost:8080
Content-Type: application/json
isBodyCorporate: true

{
  "taskUuid": "5c3588ce-20df-4cfe-baa3-39355e398f56"
}
```

**Response:**  
```json
[
  {
    "taskUuid": "5c3588ce-20df-4cfe-baa3-39355e398f56",
    "title": "Fix Broken Fence",
    "status": "PENDING",
    "quote": {
      "amount": 5000.00,
      "description": "Includes materials and labor"
    }
  },
  ...
]
```
---

### Maintenance Task Field Requirements Summary

| Field          | Create | Update | Type        | Notes                                    |
|----------------|--------|--------|-------------|------------------------------------------|
| title          | req    | opt    | String      | Title of the maintenance task            |
| des            | req    | opt    | String      | Description of the maintenance task      |
| status         | req    | opt    | String      | Current status of the task               |
| scheduled_date | req    | opt    | String      | Scheduled date (format: "YYYY-MM-DD")    |
| approved       | req    | opt    | Boolean     | Approval status                          |
| b_uuid         | req    | opt    | String/UUID | Building UUID (required)                 |
| img            | req    | opt    | String/UUID | Image UUID from images table (required) |
| t_uuid         | req    | opt    | String/UUID | Trustee UUID who created task (required)|

- **req** = required, **opt** = optional

**Required Field Notes:**
- `b_uuid`: Must reference an existing building UUID
- `img`: Must reference an existing image UUID from the images table
- `t_uuid`: Must reference an existing trustee UUID who created the maintenance task

# Quote Endpoints

## Create a Quote
**POST** `/api/quote`

**Request Body Fields:**
- `t_uuid` (**required**): UUID of the maintenance task (string/UUID)
- `c_uuid` (**required**): UUID of the contractor (string/UUID)
- `submitted_on` (**required**): Submission timestamp (string, ISO 8601 format)
- `status` (**required**): Current status of the quote (string)
- `amount` (**required**): Quote amount (decimal)
- `doc` (**required**): Document/notes related to the quote (string)

**Request Body Example:**
```json
{
  "t_uuid": "d91657fd-98e6-41d1-8afa-7773a53ed915",
  "c_uuid": "d368aef5-0f2d-46a4-b32e-1e2b79cc7e28",
  "submitted_on": "2025-06-26T10:00:00.000+02:00",
  "status": "Started",
  "amount": 1000,
  "doc": "jhihiduhidhwqidhqwudh",
  "expiry_date" : "2025-07-26T10:00:00.000+02:00", 
}
```

**Response Example (201 Created):**
```json
{
  "uuid": "b9c9cead-4f9e-4f57-99ad-8374e444c4a9",
  "t_uuid": "d91657fd-98e6-41d1-8afa-7773a53ed915",
  "c_uuid": "d368aef5-0f2d-46a4-b32e-1e2b79cc7e28",
  "submitted_on": "2025-06-26T08:00:00.000+00:00",
  "status": "Started",
  "amount": 1000.00,
  "doc": "jhihiduhidhwqidhqwudh",
  "expiry_date" : "2025-07-26T10:00:00.000+02:00", 
}
```

---

## Get All Quotes
**GET** `/api/quote`

**Description:** Returns all quotes in the system.

**Response Example:**
```json
[
  {
    "uuid": "b9c9cead-4f9e-4f57-99ad-8374e444c4a9",
    "amount": 1000.00,
    "doc": "jhihiduhidhwqidhqwudh",
    "submitted_on": "2025-06-26T08:00:00.000+00:00",
    "status": "Started",
    "t_uuid": "d91657fd-98e6-41d1-8afa-7773a53ed915",
    "c_uuid": "d368aef5-0f2d-46a4-b32e-1e2b79cc7e28",
    "expiry_date" : "2025-07-26T10:00:00.000+02:00", 
  }
]
```

---

## Get Quote by UUID
**GET** `/api/quote/{uuid}`

- **Path Variable:**  
  - `uuid` (**required**): UUID of the quote

**Description:** Returns a specific quote by its UUID.

**Response Example:**
```json
{
  "uuid": "b9c9cead-4f9e-4f57-99ad-8374e444c4a9",
  "amount": 1000.00,
  "doc": "jhihiduhidhwqidhqwudh",
  "submitted_on": "2025-06-26T08:00:00.000+00:00",
  "status": "Started",
  "t_uuid": "d91657fd-98e6-41d1-8afa-7773a53ed915",
  "c_uuid": "d368aef5-0f2d-46a4-b32e-1e2b79cc7e28",
  "expiry_date" : "2025-07-26T10:00:00.000+02:00", 
}
```

---

## Delete Quote
**DELETE** `/api/quote/{uuid}`

- **Path Variable:**  
  - `uuid` (**required**): UUID of the quote

**Description:** Deletes a specific quote.

**Response:**  
- `204 No Content` if deleted  
- `404 Not Found` if not found

---

### Quote Field Requirements Summary

| Field        | Create | Type           | Notes                                    |
|--------------|--------|----------------|------------------------------------------|
| t_uuid       | req    | String/UUID    | Maintenance task UUID (required)         |
| c_uuid       | req    | String/UUID    | Contractor UUID (required)               |
| submitted_on | req    | String         | ISO 8601 timestamp format               |
| status       | req    | String         | Current status of the quote              |
| amount       | req    | Decimal        | Quote amount                             |
| doc          | req    | String         | Document/notes related to the quote      |
| expiry_date  | req    | String         | ISO 8601 timestamp format               |

- **req** = required

**Required Field Notes:**
- `t_uuid`: Must reference an existing maintenance task UUID
- `c_uuid`: Must reference an existing contractor UUID
- `submitted_on`: Must be in ISO 8601 format (e.g., "2025-06-26T10:00:00.000+02:00")
- `expiry_date`: Must be in ISO 8601 format (e.g., "2025-06-26T10:00:00.000+02:00")

# Trustee Endpoints

## Create a Trustee
**POST** `/api/trustee`

**Request Body Example:**
```json
{
  "name": "Karabelo",
  "apikey": "0210323-2313-123",
  "email": "karabelotaole04@gmail.com",
  "phone": "013456789",
}
```

**Response Example (201 Created):**
```json
{
  "uuid": "d368aef5-0f2d-46a4-b32e-1e2b79cc7e28",
  "name": "Karabelo",
  "apikey": "0210323-2313-123",
  "email": "karabelotaole04@gmail.com",
  "phone": "013456789",
}
```

---

## Get All Trustees
**GET** `/api/trustee`

**Response Example:**
```json
[
  {
    "uuid": "d368aef5-0f2d-46a4-b32e-1e2b79cc7e28",
    "name": "Karabelo",
    "apikey": "0210323-2313-123",
    "email": "karabelotaole04@gmail.com",
    "phone": "013456789",
  }
]
```

---

## Get Trustee by UUID
**GET** `/api/trustee/{uuid}`

- **Path Variable:**  
  - `uuid` (**required**): UUID of the trustee

**Response Example:**
```json
{
  "uuid": "d368aef5-0f2d-46a4-b32e-1e2b79cc7e28",
  "name": "Karabelo",
  "apikey": "0210323-2313-123",
  "email": "karabelotaole04@gmail.com",
  "phone": "013456789",
}
```

---

# Trustee Authentication Endpoints

## Register a Trustee
**POST** `/api/trustee/auth/register`

**Request Body Example:**
```json
{
  "email": "bagoki1587@asimarif.com",
  "password": "Test@1234",
  "contactNumber": "0672687235"
}
```

**Response Example (200 OK):**
```json
{
  "message": "Registration successful, please verify your email.",
  "username": "bagoki15871750943941252"
}
```

> **Note:** Store the username for the next step (email confirmation).

---

## Confirm Email
**POST** `/api/trustee/auth/confirm`

**Request Body Example:**
```json
{
  "username": "bagoki15871750941249379",
  "code": "621074"
}
```

**Response Example:**
```json
{
  "message": "Account confirmed."
}
```

---

## Login
**POST** `/api/trustee/auth/login`

**Request Body Example:**
```json
{
  "email": "bagoki1587@asimarif.com",
  "password": "Test@1234"
}
```

**Response Example (200 OK):**
```json
{
  "idToken": "[some long token]",
  "accessToken": "[some long token]",
  "refreshToken": "[some long token]",
  "userType": "trustee",
  "userId": "5b029df7-1e95-439e-89d9-bb2bdc4a3766"
}
```

# Invites endpoints 

## create invite 
**POST** `/api/invites`

**Request Body Example:**
```json
{
  "trusteeUuid": "5f67ec9a-84f2-4d72-a6a3-48393f8a0456",
  "coporateUuid": "5ad98526-6f94-4ce0-acf9-31e99590f8e2"
}
```
---

## Get by invite Uuid
**GET** `/api/invites/{uuid}`

---

## Get all invites for trustee
**GET** `/api/invites/trustee/{uuid}`

---

## Accept invite
**PUT** `/api/invites/{uuid}/status?status=ACCEPTED`

---

## Get all trustees in body corporate 
**GET** /api/invites/body-corporate/{corp-uuid}/accepted-trustees

---

## Get all 
**GET** `/api/invites`

**RESPONSE**
```json
[
    {
        "inviteUuid": "36f15795-5e65-46b4-b569-aa630ed7e702",
        "trusteeUuid": "4d33a2ce-110d-403a-9172-73baca2e5664",
        "coporateUuid": "5ad98526-6f94-4ce0-acf9-31e99590f8e2",
        "status": "ACCEPTED",
        "invitedOn": 1752581150612
    },
    {
        "inviteUuid": "23f5545b-dba8-4619-9ddc-a8426e8d5bb2",
        "trusteeUuid": "5f67ec9a-84f2-4d72-a6a3-48393f8a0456",
        "coporateUuid": "5ad98526-6f94-4ce0-acf9-31e99590f8e2",
        "status": "PENDING",
        "invitedOn": 1753629358177
    }
]
```

---

# Life cycle cost

## Create Life cycle cost
**POST** `/api/lifecycle-cost`

**Request Body Example:**
```json
{
  "coporateUuid": "5ad98526-6f94-4ce0-acf9-31e99590f8e2",
  "type": "Roof",
  "description": "Needs repainting",
  "condition": "Worn",
  "timeframe": "6 months",
  "estimatedCost": 15000.00
}

```
---

## GET BY ID 
**GET** `/api/lifecycle-cost/{uuid}`

---

## GET By Coporate UUID
**GET** `/api/lifecycle-cost/coporate/{uuid}`

---

## Update
**PUT** `/api/lifecycle-cost/{uuid}`

**Request Body Example:**
```json
{
  "type": "Roof",
  "description": "Urgent repair needed",
  "condition": "Critical",
  "timeframe": "1 month",
  "estimatedCost": 25000.00
}

```
---

# Voting system 

## create voting session 
**POST** `/api/vote/session`

**Request Body Example:**
```json
{
    "coporateUuid" : "5ad98526-6f94-4ce0-acf9-31e99590f8e2",
    "taskUuid": "d05fed07-3568-4b69-8bad-6b9dedf8515b",
    "votingEndsAt": "2025-07-21T18:40:00"   
}
```
---

## Submit Vote 
**POST** `/api/vote`

**Request Body Example:**
```json
{
    "sessionUuid": "166fb380-ac2e-4466-951a-f7f47516be48",
    "quoteUuid": "2bb7bd53-53a3-427e-95cd-eb9bcd3addb6",
    "voterUuid": "5ad98526-6f94-4ce0-acf9-31e99590f8e2",
    "isTrustee": false,
    "voteFor": true
}
```
---

## Results 
**GET** `/api/vote/session/{session uuid}/results`

---

## Get all sessions
**GET** `/api/vote/sessions`

---

## get by task uuid by session uuid 
**GET** `/api/vote/session/{session uuid}/task`

---

## get session uuid by task uuid 
**GET** `/api/vote/task/{task uuid}/session`

---

# task progress

## add task work 
**POST** `/api/task-progress`

**Request Body Example:**
```json
{
    "contractorUuid": "288b32c8-6123-497c-bf7e-e8fa99a075a8",
    "taskUuid": "5c3588ce-20df-4cfe-baa3-39355e398f56",
    "imageId" : "f88aad9c-93e9-4459-a9d6-38af1c7e6b9a",
    "workDescription" : "painted undercoat",
    "inventoryUsageUuid" : "d246040f-ac07-491f-bf54-b7e77bbe6d8b",
    "quantityUsed" : "5"
}
```

**Response:200**
```json
{
    "progressUuid": "93ae932e-7eb2-4f7b-aa3a-197f9714e039",
    "submissionDate": 1754737940.029881833,
    "contractorUuid": "288b32c8-6123-497c-bf7e-e8fa99a075a8",
    "taskUuid": "5c3588ce-20df-4cfe-baa3-39355e398f56",
    "imageId": "f88aad9c-93e9-4459-a9d6-38af1c7e6b9a",
    "progressPercentage": 0,
    "workDescription": "painted undercoat",
    "inventoryUsageUuid": "d246040f-ac07-491f-bf54-b7e77bbe6d8b",
    "quantityUsed": 5,
    "remarks": null,
    "lastUpdated": 1754737940.029940224
}
```
---

## Get by task id :
**Get** `api/task-progress/{progressUuid}`

**Response:200**
```json
{
    "progressUuid": "93ae932e-7eb2-4f7b-aa3a-197f9714e039",
    "submissionDate": 1754737940.029881833,
    "contractorUuid": "288b32c8-6123-497c-bf7e-e8fa99a075a8",
    "taskUuid": "5c3588ce-20df-4cfe-baa3-39355e398f56",
    "imageId": "f88aad9c-93e9-4459-a9d6-38af1c7e6b9a",
    "progressPercentage": 0,
    "workDescription": "painted undercoat",
    "inventoryUsageUuid": "d246040f-ac07-491f-bf54-b7e77bbe6d8b",
    "quantityUsed": 5,
    "remarks": null,
    "lastUpdated": 1754737940.029940224
}
```
---

## Get by task uuid:
**Get** `api/task-progress/task/{taskUuid}`

**Response:200**
```json
{
    "progressUuid": "93ae932e-7eb2-4f7b-aa3a-197f9714e039",
    "submissionDate": 1754737940.029881833,
    "contractorUuid": "288b32c8-6123-497c-bf7e-e8fa99a075a8",
    "taskUuid": "5c3588ce-20df-4cfe-baa3-39355e398f56",
    "imageId": "f88aad9c-93e9-4459-a9d6-38af1c7e6b9a",
    "progressPercentage": 0,
    "workDescription": "painted undercoat",
    "inventoryUsageUuid": "d246040f-ac07-491f-bf54-b7e77bbe6d8b",
    "quantityUsed": 5,
    "remarks": null,
    "lastUpdated": 1754737940.029940224
}
```
---

## Update 
**PUT** `api/task-progress/task/{taskUuid}`

**Request body example:**
```json
{
    "progressPercentage" : 60,
    "imageId" : "f88aad9c-93e9-4459-a9d6-38af1c7e6b9a",
    "workDescription" : "finished painting 3 out of the 5 walls "
}
```

**Response:200**
```json
{
    "progressUuid": "93ae932e-7eb2-4f7b-aa3a-197f9714e039",
    "submissionDate": 1754737940.029882000,
    "contractorUuid": "288b32c8-6123-497c-bf7e-e8fa99a075a8",
    "taskUuid": "5c3588ce-20df-4cfe-baa3-39355e398f56",
    "imageId": "f88aad9c-93e9-4459-a9d6-38af1c7e6b9a",
    "progressPercentage": 60,
    "workDescription": "finished painting 3 out of the 5 walls ",
    "inventoryUsageUuid": "d246040f-ac07-491f-bf54-b7e77bbe6d8b",
    "quantityUsed": 5,
    "remarks": null,
    "lastUpdated": 1754739343.323560387
}
```
### updatable fileds :
```
    val progressPercentage: BigDecimal,
    val workDescription: String,
    val imageId: String,
    val quantityUsed: Int,
    val remarks: String
```

## Delete task progress :
**DELETE** `/api/task-progress/{progressUuid}`
**Response:204** 


# PDF Upload Endpoints

## Generate Presigned Upload URL
**GET** `/api/upload/presigned-upload/{filename}`

- **Path Variable:**  
  - `filename` (**required**): Name of the PDF file to be uploaded (string)

**Description:** Generates a presigned URL for uploading a PDF file directly to S3. The URL is valid for 15 minutes.

**Response Example (200 OK):**
```json
{
  "uploadUrl": "https://your-bucket.s3.amazonaws.com/uploads-123e4567-e89b-12d3-a456-426614174000-document.pdf?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=...",
  "fileKey": "uploads-123e4567-e89b-12d3-a456-426614174000-document.pdf",
  "id": "123e4567-e89b-12d3-a456-426614174000"
}
```

**Frontend Usage Steps:**
1. Call this endpoint with the filename
2. Use the returned `uploadUrl` to upload the file directly to S3 via PUT request
3. After successful upload, call the notify endpoint with the returned `id`, `filename`, and `fileKey`

---

## Notify Upload Complete
**POST** `/api/upload/notify-upload/{id}/{filename}/{key}`

- **Path Variables:**  
  - `id` (**required**): UUID returned from the presigned upload URL generation (string/UUID)
  - `filename` (**required**): Original filename of the uploaded PDF (string)
  - `key` (**required**): S3 object key returned from presigned upload URL generation (string)

**Description:** Notifies the system that a file upload has been completed and saves the metadata to the database.

**Response Example (200 OK):**
```json
"Upload metadata saved."
```

**Frontend Usage Steps:**
1. After successfully uploading to S3 using the presigned URL
2. Call this endpoint with the `id`, `filename`, and `fileKey` from the initial presigned URL response
3. The system will save the PDF metadata for future retrieval

---

## Get Presigned Download URL
**GET** `/api/upload/presigned/{id}`

- **Path Variable:**  
  - `id` (**required**): UUID of the uploaded PDF (string/UUID)

**Description:** Generates a presigned URL for downloading a previously uploaded PDF file. The URL is valid for 10 minutes.

**Response Example (200 OK):**
```
https://your-bucket.s3.amazonaws.com/uploads-123e4567-e89b-12d3-a456-426614174000-document.pdf?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=...
```

**Response:**  
- `200 OK` with presigned URL as plain text
- `404 Not Found` if PDF with given ID doesn't exist

**Frontend Usage Steps:**
1. Call this endpoint with the PDF's ID
2. Use the returned presigned URL to download/display the PDF
3. The URL expires after 10 minutes, so generate a new one if needed

---

### PDF Upload Flow Summary

| Step | Method | Endpoint | Purpose |
|------|--------|----------|---------|
| 1 | GET | `/api/upload/presigned-upload/{filename}` | Get upload URL and metadata |
| 2 | PUT | `{uploadUrl from step 1}` | Upload file directly to S3 |
| 3 | POST | `/api/upload/notify-upload/{id}/{filename}/{key}` | Save metadata to database |
| 4 | GET | `/api/upload/presigned/{id}` | Get download URL when needed |

**Complete Frontend Implementation Example:**

```javascript
// Step 1: Get presigned upload URL
async function uploadPDF(file) {
  // Get presigned URL
  const presignedResponse = await fetch(`/api/upload/presigned-upload/${file.name}`);
  const { uploadUrl, fileKey, id } = await presignedResponse.json();
  
  // Step 2: Upload file to S3
  const uploadResponse = await fetch(uploadUrl, {
    method: 'PUT',
    body: file,
    headers: {
      'Content-Type': 'application/pdf'
    }
  });
  
  if (uploadResponse.ok) {
    // Step 3: Notify backend
    await fetch(`/api/upload/notify-upload/${id}/${file.name}/${fileKey}`, {
      method: 'POST'
    });
    
    return id; // Save this ID for later retrieval
  }
}

// Step 4: Get download URL when needed
async function getPDFUrl(pdfId) {
  const response = await fetch(`/api/upload/presigned/${pdfId}`);
  return await response.text(); // Returns the presigned URL
}
```

**Important Notes:**
- All presigned URLs have expiration times (15 minutes for upload, 10 minutes for download)
- Files are automatically prefixed with "uploads-{uuid}-" in S3
- Only PDF files are supported (Content-Type: application/pdf)
- The system expects the exact filename, ID, and key values returned from the initial presigned URL generation


# Rating Endpoints

## Create a Rating
**POST** `/api/rating`

**Request Body Fields:**
- `contractorUuid` (**required**): UUID of the contractor being rated (string/UUID)
- `comment` (**required**): Rating comment/feedback (string)
- `rating` (**required**): Rating value (number)
- `taskUuid` (**required**): UUID of the related task (string/UUID)
- `trusteeUuid` (**required**): UUID of the trustee giving the rating (string/UUID)

**Request Body Example:**
```json
{
  "contractorUuid": "d368aef5-0f2d-46a4-b32e-1e2b79cc7e28",
  "comment": "Excellent work, completed on time and very professional",
  "rating": 5.0,
  "taskUuid": "d91657fd-98e6-41d1-8afa-7773a53ed915",
  "trusteeUuid": "a1b2c3d4-5e6f-7g8h-9i0j-k1l2m3n4o5p6"
}
```

**Response Example (200 OK):**
```json
{
  "uuid": "b9c9cead-4f9e-4f57-99ad-8374e444c4a9",
  "contractorUuid": "d368aef5-0f2d-46a4-b32e-1e2b79cc7e28",
  "comment": "Excellent work, completed on time and very professional",
  "rating": 5.0,
  "taskUuid": "d91657fd-98e6-41d1-8afa-7773a53ed915",
  "trusteeUuid": "a1b2c3d4-5e6f-7g8h-9i0j-k1l2m3n4o5p6"
}
```

---

## Get All Ratings
**GET** `/api/rating`

**Description:** Returns all ratings in the system.

**Response Example:**
```json
[
  {
    "uuid": "b9c9cead-4f9e-4f57-99ad-8374e444c4a9",
    "contractorUuid": "d368aef5-0f2d-46a4-b32e-1e2b79cc7e28",
    "comment": "Excellent work, completed on time and very professional",
    "rating": 5.0,
    "taskUuid": "d91657fd-98e6-41d1-8afa-7773a53ed915",
    "trusteeUuid": "a1b2c3d4-5e6f-7g8h-9i0j-k1l2m3n4o5p6"
  },
  {
    "uuid": "c8d9e0f1-2a3b-4c5d-6e7f-8g9h0i1j2k3l",
    "contractorUuid": "d368aef5-0f2d-46a4-b32e-1e2b79cc7e28",
    "comment": "Good work but could improve communication",
    "rating": 4.0,
    "taskUuid": "e2f3g4h5-6i7j-8k9l-0m1n-2o3p4q5r6s7t",
    "trusteeUuid": "a1b2c3d4-5e6f-7g8h-9i0j-k1l2m3n4o5p6"
  }
]
```

---

## Get Rating by Contractor UUID
**GET** `/api/rating/{uuid}`

- **Path Variable:**  
  - `uuid` (**required**): UUID of the contractor to get ratings for

**Description:** Returns ratings for a specific contractor by their UUID.

**Response Example (200 OK):**
```json
{
  "uuid": "b9c9cead-4f9e-4f57-99ad-8374e444c4a9",
  "contractorUuid": "d368aef5-0f2d-46a4-b32e-1e2b79cc7e28",
  "comment": "Excellent work, completed on time and very professional",
  "rating": 5.0,
  "taskUuid": "d91657fd-98e6-41d1-8afa-7773a53ed915",
  "trusteeUuid": "a1b2c3d4-5e6f-7g8h-9i0j-k1l2m3n4o5p6"
}
```

**Response:**  
- `200 OK` with rating data
- `404 Not Found` if no rating found for the contractor

---

## Get Average Rating for Contractor
**GET** `/api/rating/avg/{uuid}`

- **Path Variable:**  
  - `uuid` (**required**): UUID of the contractor to get average rating for

**Description:** Returns the average rating for a specific contractor.

**Response Example (200 OK):**
```json
4.2
```

**Response:**  
- `200 OK` with average rating as a number
- `404 Not Found` if no ratings found for the contractor

---

## Update Rating
**PUT** `/api/rating/{uuid}`

- **Path Variable:**  
  - `uuid` (**required**): UUID of the rating to update

**Request Body Example:**
```json
{
  "uuid": "b9c9cead-4f9e-4f57-99ad-8374e444c4a9",
  "contractorUuid": "d368aef5-0f2d-46a4-b32e-1e2b79cc7e28",
  "comment": "Updated: Excellent work, completed on time and very professional",
  "rating": 4.5,
  "taskUuid": "d91657fd-98e6-41d1-8afa-7773a53ed915",
  "trusteeUuid": "a1b2c3d4-5e6f-7g8h-9i0j-k1l2m3n4o5p6"
}
```

**Description:** Updates an existing rating with new data.

**Response Example (200 OK):**
```json
{
  "uuid": "b9c9cead-4f9e-4f57-99ad-8374e444c4a9",
  "contractorUuid": "d368aef5-0f2d-46a4-b32e-1e2b79cc7e28",
  "comment": "Updated: Excellent work, completed on time and very professional",
  "rating": 4.5,
  "taskUuid": "d91657fd-98e6-41d1-8afa-7773a53ed915",
  "trusteeUuid": "a1b2c3d4-5e6f-7g8h-9i0j-k1l2m3n4o5p6"
}
```

---

## Delete Rating
**DELETE** `/api/rating/{uuid}`

- **Path Variable:**  
  - `uuid` (**required**): UUID of the rating to delete

**Description:** Deletes a specific rating.

**Response:**  
- `204 No Content` if deleted successfully
- `404 Not Found` if rating not found

---

### Rating Field Requirements Summary

| Field          | Create | Update | Type           | Notes                                    |
|----------------|--------|--------|----------------|------------------------------------------|
| contractorUuid | req    | opt    | String/UUID    | UUID of contractor being rated           |
| comment        | req    | opt    | String         | Rating comment/feedback                  |
| rating         | req    | opt    | Number         | Rating value (typically 1-5 scale)      |
| taskUuid       | req    | opt    | String/UUID    | UUID of related maintenance task         |
| trusteeUuid    | req    | opt    | String/UUID    | UUID of trustee giving the rating        |

- **req** = required
- **opt** = optional

**Required Field Notes:**
- `contractorUuid`: Must reference an existing contractor UUID
- `taskUuid`: Must reference an existing maintenance task UUID  
- `trusteeUuid`: Must reference an existing trustee UUID
- `rating`: Should typically be within a defined range (e.g., 1-5)
- `comment`: Provides context and feedback for the rating

**Frontend Usage Examples:**

```javascript
// Create a new rating
async function createRating(ratingData) {
  const response = await fetch('/api/rating', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json'
    },
    body: JSON.stringify(ratingData)
  });
  return await response.json();
}

// Get contractor's average rating
async function getContractorAverageRating(contractorUuid) {
  const response = await fetch(`/api/rating/avg/${contractorUuid}`);
  if (response.ok) {
    return await response.json(); // Returns number
  }
  return null; // No ratings found
}

// Get all ratings for a contractor
async function getContractorRatings(contractorUuid) {
  const response = await fetch(`/api/rating/${contractorUuid}`);
  if (response.ok) {
    return await response.json();
  }
  return null; // No ratings found
}
```

# Contractor Corporate Endpoints

## Create a Contractor Corporate Association
**POST** `/api/contractorCorporate`

**Request Body Fields:**
- `contractorUuid` (**required**): UUID of the contractor (string/UUID)
- `bodyCorporateUuid` (**required**): UUID of the body corporate (string/UUID)

**Request Body Example:**
```json
{
  "contractorUuid": "d368aef5-0f2d-46a4-b32e-1e2b79cc7e28",
  "bodyCorporateUuid": "a1b2c3d4-5e6f-7g8h-9i0j-k1l2m3n4o5p6"
}
```

**Response Example (200 OK):**
```json
{
  "uuid": "b9c9cead-4f9e-4f57-99ad-8374e444c4a9",
  "contractorUuid": "d368aef5-0f2d-46a4-b32e-1e2b79cc7e28",
  "bodyCorporateUuid": "a1b2c3d4-5e6f-7g8h-9i0j-k1l2m3n4o5p6"
}
```

---

## Get All Contractor Corporate Associations
**GET** `/api/contractorCorporate`

**Description:** Returns all contractor-corporate associations in the system.

**Response Example:**
```json
[
  {
    "uuid": "b9c9cead-4f9e-4f57-99ad-8374e444c4a9",
    "contractorUuid": "d368aef5-0f2d-46a4-b32e-1e2b79cc7e28",
    "bodyCorporateUuid": "a1b2c3d4-5e6f-7g8h-9i0j-k1l2m3n4o5p6"
  },
  {
    "uuid": "c8d9e0f1-2a3b-4c5d-6e7f-8g9h0i1j2k3l",
    "contractorUuid": "e479bgh6-1g3h-47b5-c43f-9h0i1j2k3l4m",
    "bodyCorporateUuid": "f5g6h7i8-9j0k-1l2m-3n4o-5p6q7r8s9t0u"
  }
]
```

---

## Get Contractor Corporate Association by UUID
**GET** `/api/contractorCorporate/{uuid}`

- **Path Variable:**  
  - `uuid` (**required**): UUID of the contractor corporate association

**Description:** Returns a specific contractor-corporate association by its UUID.

**Response Example (200 OK):**
```json
{
  "uuid": "b9c9cead-4f9e-4f57-99ad-8374e444c4a9",
  "contractorUuid": "d368aef5-0f2d-46a4-b32e-1e2b79cc7e28",
  "bodyCorporateUuid": "a1b2c3d4-5e6f-7g8h-9i0j-k1l2m3n4o5p6"
}
```

**Response:**  
- `200 OK` with association data
- `404 Not Found` if association not found

---

## Get Contractor Corporate Associations by Contractor UUID
**GET** `/api/contractorCorporate/list/{uuid}`

- **Path Variable:**  
  - `uuid` (**required**): UUID of the contractor to get corporate associations for

**Description:** Returns all corporate associations for a specific contractor.

**Response Example (200 OK):**
```json
[
  {
    "uuid": "b9c9cead-4f9e-4f57-99ad-8374e444c4a9",
    "contractorUuid": "d368aef5-0f2d-46a4-b32e-1e2b79cc7e28",
    "bodyCorporateUuid": "a1b2c3d4-5e6f-7g8h-9i0j-k1l2m3n4o5p6"
  },
  {
    "uuid": "c8d9e0f1-2a3b-4c5d-6e7f-8g9h0i1j2k3l",
    "contractorUuid": "d368aef5-0f2d-46a4-b32e-1e2b79cc7e28",
    "bodyCorporateUuid": "f5g6h7i8-9j0k-1l2m-3n4o-5p6q7r8s9t0u"
  }
]
```

**Response:**  
- `200 OK` with list of associations
- `404 Not Found` if no associations found for the contractor

---

## Get Contractor UUIDs by Body Corporate
**GET** `/api/contractorCorporate/contractors/{bodyCorporateUuid}`

- **Path Variable:**  
  - `bodyCorporateUuid` (**required**): UUID of the body corporate to get contractors for

**Description:** Returns a list of contractor UUIDs associated with a specific body corporate.

**Response Example (200 OK):**
```json
[
  "d368aef5-0f2d-46a4-b32e-1e2b79cc7e28",
  "e479bgh6-1g3h-47b5-c43f-9h0i1j2k3l4m",
  "f5a7c9e1-3f5h-59c7-e51j-1l3n5p7r9t1v"
]
```

**Response:**  
- `200 OK` with array of contractor UUIDs
- `404 Not Found` if no contractors found for the body corporate

---

## Update Contractor Corporate Association
**PUT** `/api/contractorCorporate/{uuid}`

- **Path Variable:**  
  - `uuid` (**required**): UUID of the association to update

**Request Body Example:**
```json
{
  "uuid": "b9c9cead-4f9e-4f57-99ad-8374e444c4a9",
  "contractorUuid": "d368aef5-0f2d-46a4-b32e-1e2b79cc7e28",
  "bodyCorporateUuid": "a1b2c3d4-5e6f-7g8h-9i0j-k1l2m3n4o5p6"
}
```

**Description:** Updates an existing contractor-corporate association.

**Response Example (200 OK):**
```json
{
  "uuid": "b9c9cead-4f9e-4f57-99ad-8374e444c4a9",
  "contractorUuid": "d368aef5-0f2d-46a4-b32e-1e2b79cc7e28",
  "bodyCorporateUuid": "a1b2c3d4-5e6f-7g8h-9i0j-k1l2m3n4o5p6"
}
```

---

## Delete Contractor Corporate Association
**DELETE** `/api/contractorCorporate/{uuid}`

- **Path Variable:**  
  - `uuid` (**required**): UUID of the association to delete

**Description:** Deletes a specific contractor-corporate association.

**Response:**  
- `204 No Content` if deleted successfully
- `404 Not Found` if association not found

---

### Contractor Corporate Field Requirements Summary

| Field              | Create | Update | Type           | Notes                                    |
|--------------------|--------|--------|----------------|------------------------------------------|
| contractorUuid     | req    | opt    | String/UUID    | UUID of the contractor                   |
| bodyCorporateUuid  | req    | opt    | String/UUID    | UUID of the body corporate               |

- **req** = required
- **opt** = optional

**Required Field Notes:**
- `contractorUuid`: Must reference an existing contractor UUID
- `bodyCorporateUuid`: Must reference an existing body corporate UUID
- This entity manages many-to-many relationships between contractors and body corporates

**Frontend Usage Examples:**

```javascript
// Create contractor-corporate association
async function createContractorCorporateAssociation(contractorUuid, bodyCorporateUuid) {
  const response = await fetch('/api/contractorCorporate', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json'
    },
    body: JSON.stringify({
      contractorUuid: contractorUuid,
      bodyCorporateUuid: bodyCorporateUuid
    })
  });
  return await response.json();
}

// Get all contractors for a body corporate
async function getContractorsForBodyCorporate(bodyCorporateUuid) {
  const response = await fetch(`/api/contractorCorporate/contractors/${bodyCorporateUuid}`);
  if (response.ok) {
    return await response.json(); // Returns array of contractor UUIDs
  }
  return []; // No contractors found
}

// Get all body corporates for a contractor
async function getBodyCorporatesForContractor(contractorUuid) {
  const response = await fetch(`/api/contractorCorporate/list/${contractorUuid}`);
  if (response.ok) {
    return await response.json(); // Returns array of associations
  }
  return []; // No associations found
}

// Delete contractor-corporate association
async function removeContractorFromBodyCorporate(associationUuid) {
  const response = await fetch(`/api/contractorCorporate/${associationUuid}`, {
    method: 'DELETE'
  });
  return response.status === 204; // Returns true if successful
}
```

**Common Use Cases:**
- **Assign contractors to body corporates**: Create associations to define which contractors can work for which body corporates
- **Get available contractors**: Use the `/contractors/{bodyCorporateUuid}` endpoint to find all contractors available for a specific body corporate
- **Contractor portfolio**: Use the `/list/{uuid}` endpoint to see all body corporates a contractor is associated with
- **Manage permissions**: Create/delete associations to control contractor access to different body corporates